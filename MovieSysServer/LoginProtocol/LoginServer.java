...
//			protocol = new Protocol();			// 새 Protocol 객체 생성 (기본 생성자)
			...
			switch(packetType){
				...
				case Protocol.PT_REQ_LOGIN:		// 로그인 정보 수신 
					...
					if(result.equals("false")){ //로그인 실패
						...
						//영화/영화관/내정보 창 띄우기
						if(result.equals("C")){
							protocol = new Protocol(Protocol.PT_RES_LOGIN,Protocol.CODE_PT_RES_LOGIN_CUS_SUCCESS);//코드1 	고객 로그인 요청
							os.write(protocol.getPacket());
							break;
						}
						...
						//영화관 관리/영화 관리/ 상영 관리/ 통계정보/계좌관리 창 띄우기
							...
							protocol = new Protocol(Protocol.PT_RES_LOGIN,Protocol.CODE_PT_RES_LOGIN_ADMIN_SUCCESS);//코드2	관리자 로그인 요청
							os.write(protocol.getPacket());
							break;
						}
		...
		// 					protocol = new Protocol(Protocol.PT_RES_LOOKUP,Protocol.CODE_PT_RES_LOOKUP_FIND_CUS_ID_NO);//가입된 아이디가 없어서 다시 조회 요청
		// 					os.write(protocol.getPacket());					//코드 2
		// 					break;
		// 				}
		// 				else{ // 아이디 보내줌
		// 					protocol = new Protocol(Protocol.PT_RES_LOOKUP,Protocol.CODE_PT_RES_LOOKUP_FIND_CUS_ID_OK); // 코드 1
		// 					protocol.setID(id);
		// 					os.write(protocol.getPacket());
		// 					break;
		...
		// 					if(idCheck(id)==false){		//회원가입할 때 id 중복체크
		// 						protocol = new Protocol(Protocol.PT_RES_SIGNUP,Protocol.CODE_PT_RES_UPDATE_ADD_MEM_NO);//중복에 걸려서 다시 회원가입 요청
		// 						os.write(protocol.getPacket());
		// 						break;
		// 					}
		// 					else{
		...
		// 						protocol= new Protocol(Protocol.PT_RES_SIGNUP,Protocol.CODE_PT_RES_UPDATE_ADD_MEM_OK);
		// 						os.write(protocol.getPacket());
