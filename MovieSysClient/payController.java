        ...
        pay_btn.setOnAction(event -> {
            ...
            if (confirm.isSelected())
                try {
                    ...
                    if ((protocol.getProtocolType() == Protocol.PT_RES_UPDATE) && (protocol.getProtocolCode() == Protocol.CODE_PT_RES_UPDATE_ADD_PAY_RESV_OK)) { // 성공
                        ...
                        primaryStage.setScene(scene2);
                            ...
                            && (protocol.getProtocolCode() == Protocol.CODE_PT_RES_UPDATE_ADD_PAY_RESV_NO2)) { // 돈모자람/좌석은 남아있음
                        Stage dialog = new Stage(StageStyle.UTILITY);
                        dialog.initModality(Modality.WINDOW_MODAL);
                        dialog.initOwner(primaryStage);
                        ...
                        primaryStage.setScene(scene2);
                            ...
                            && (protocol.getProtocolCode() == Protocol.CODE_PT_RES_UPDATE_ADD_PAY_RESV_NO1)) { // 좌석x 돈있음
                        Stage dialog = new Stage(StageStyle.UTILITY);
                        dialog.initModality(Modality.WINDOW_MODAL);
                        dialog.initOwner(primaryStage);
                        dialog.setTitle("Error");
